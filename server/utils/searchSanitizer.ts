...
export const sanitizeSearchQuery = (query: string | undefined): string | undefined => {
  ...
  if (sanitized.length > SEARCH_CONFIG.MAX_QUERY_LENGTH) {
    sanitized = sanitized.substring(0, SEARCH_CONFIG.MAX_QUERY_LENGTH);
  }

  // Block malicious patterns
  for (const pattern of SEARCH_CONFIG.BLOCKED_PATTERNS) {
    if (pattern.test(sanitized)) {
      // Return undefined for blocked patterns - safer than trying to clean
      return undefined;
    }
  }
...
export const validateAndSanitizeSearchQuery = (query: {
  ...
  try {
    ...
    const sanitizedQ = sanitizeSearchQuery(query.q);
    
    // If query was provided but sanitization failed, it's likely malicious
    if (query.q && query.q.trim() && !sanitizedQ) {
      return {
        isValid: false,
        error: 'Invalid search query. Please avoid special characters and keep queries under 100 characters.'
      };
    }
